...
GRAFANA_URL_DEFAULT=""
SCREENSHOT_DIR_DEFAULT="${PROJECT_DIR}/screenshots"
WAIT_TIME_SECONDS_DEFAULT=60
WIDTH_DEFAULT=1600
HEIGHT_DEFAULT=900
PROFILE_DIR_DEFAULT="${PROJECT_DIR}/.grafana_auto_profile"
HEADLESS_DEFAULT=1
CONFIG_PATH_LOCAL_DEFAULT="${PROJECT_DIR}/grafana_auto.yaml"
CONFIG_PATH_LOCAL_ALT_DEFAULT="${PROJECT_DIR}/grafana_auto.local.yaml"
CONFIG_PATH_TEMPLATE_DEFAULT="${APP_DIR}/configs/grafana_auto.demo.yaml"
...
Notes:
  - --target 必须跟一个 name，例如：--target home_connect
...
if [[ ! -d "${DEPS_DIR}/node_modules/playwright-core" ]] || [[ ! -d "${DEPS_DIR}/node_modules/pngjs" ]] || [[ ! -d "${DEPS_DIR}/node_modules/yaml" ]]; then
  if [[ -d "${PROJECT_DIR}/.grafana_auto_deps/node_modules/playwright-core" ]] && [[ -d "${PROJECT_DIR}/.grafana_auto_deps/node_modules/pngjs" ]] && [[ -d "${PROJECT_DIR}/.grafana_auto_deps/node_modules/yaml" ]]; then
...
if [[ ${USE_CONFIG} -eq 1 ]] || [[ -f "${CONFIG_PATH}" && ${CLI_OVERRIDE} -eq 0 ]]; then
  ...
  if [[ -n "${TARGET_NAME}" ]]; then
    NODE_PATH="${DEPS_DIR}/node_modules" node "${APP_DIR}/src/grafana_auto_runner.js" \
...
if [[ -n "${PROFILE_NAME}" ]]; then
  NODE_PATH="${DEPS_DIR}/node_modules" node "${APP_DIR}/src/grafana_screenshot.js" \
    --url "${GRAFANA_URL}" \
