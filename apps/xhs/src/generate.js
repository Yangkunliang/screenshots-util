...
const articles = [
  ...
  {
    title: "Redis 缓存雪崩、击穿、穿透：别再背八股文了，看实战",
    body:
      [
        "这三个词面试总问，但生产环境里到底长啥样？简单说：雪崩是“死一片”，击穿是“死一个点”，穿透是“一直穿”。",
        "",
        "一、缓存雪崩 (Cache Avalanche)",
        "现象：大量 key 在同一时间过期，或者 Redis 挂了。请求全部打到数据库，数据库瞬间崩盘。",
        "对策：",
        "1. 过期时间加随机值（比如 10分钟 + random(1-60s)），避免集体过期。",
        "2. Redis 高可用（Sentinel/Cluster），避免单点故障。",
        "3. 限流降级：数据库扛不住时，直接返回空或默认值，保住 DB。",
        "",
        "二、缓存击穿 (Cache Breakdown)",
        "现象：一个热点 key（比如秒杀商品）过期了，此时并发量巨大，所有请求同时打到 DB。",
        "对策：",
        "1. 互斥锁：发现 key 没了，先加锁，只有拿到锁的线程去查 DB 回填缓存，其他线程等待。",
        "2. 逻辑过期：key 不设 TTL，但在 value 里存过期时间。发现过期了，异步起线程去更新，旧请求先返回旧数据。",
        "",
        "三、缓存穿透 (Cache Penetration)",
        "现象：查一个根本不存在的数据（ID=-1），缓存没命中，DB 也没有。黑客可以用这个把 DB 刷挂。",
        "对策：",
        "1. 缓存空对象：查不到也存个 null 到 Redis（设短 TTL）。",
        "2. 布隆过滤器 (Bloom Filter)：请求前先判断是否存在，不存在直接拦截。",
        "",
        "总结：",
        "- 雪崩防“大面积过期”",
        "- 击穿防“热点失效”",
